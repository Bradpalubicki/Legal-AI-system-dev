...
# Simple service detection without complex imports
...
async def test_basic_openai():
    ...
    try:
        ...
        print("OpenAI library not available - skipping test")
        print()
        return
    
    api_key = os.getenv("OPENAI_API_KEY")
    if not api_key:
        print("No OpenAI API key - skipping test")
        print()
        return
    
    if not (api_key.startswith('sk-') and len(api_key) > 20):
        print("Mock OpenAI key detected - would use mock responses")
        print()
        return
...
async def test_basic_anthropic():
    ...
    try:
        ...
        print("Anthropic library not available - skipping test")
        print()
        return
    
    api_key = os.getenv("ANTHROPIC_API_KEY")
    if not api_key:
        print("No Anthropic API key - skipping test")
        print()
        return
    
    if not (api_key.startswith('sk-ant-') and len(api_key) > 30):
        print("Mock Anthropic key detected - would use mock responses")
        print()
        return
    ...
    try:
        ...
        print(f"Anthropic API Test: SUCCESS")
        print(f"   Response: {result}")
...
- Risk Level: Medium - standard commercial terms""",
    ...
    anthropic_review = {
        ...
        "review": f"""Review of OpenAI Analysis:
...
Overall assessment: The analysis is accurate but could benefit from additional risk factors.""",
    ...
    print(f"Anthropic review completed")
...
def generate_recommendations(status):
    ...
    if status["pipeline"] == "FULL_LIVE_PIPELINE":
        recommendations.append("All services configured! Ready for production use.")
