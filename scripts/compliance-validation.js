 ...
 * 
 * Usage: node scripts/compliance-validation.js
 */

const fs = require('fs').promises;
const path = require('path');

// Simple chalk-like coloring for Windows compatibility
const colors = {
...
class ComplianceValidator {
  ...
  async runFullValidation() {
    console.log(colors.blue('🔍 Starting Legal AI System Compliance Validation\n'));
  ...
  async check1_AIOutputDisclaimers() {
    console.log(colors.yellow('📋 Check 1: AI Output Disclaimers'));
    ...
    try {
      ...
      check.status = 'fail';
      check.details.push(`❌ Error during validation: ${error.message}`);
    }
  ...
  async check2_NoAdviceLanguage() {
    console.log(colors.yellow('⚖️  Check 2: No Legal Advice Language'));
  ...
  async check3_AttorneyReviewFlags() {
    console.log(colors.yellow('👨‍⚖️ Check 3: Attorney Review Flags'));
  ...
  async check4_StateRulesApplication() {
    console.log(colors.yellow('🏛️  Check 4: State Rules Application'));
    ...
    try {
      ...
      if (statesCoverage >= 75 && stateRuleManager.length > 0) {
        check.status = 'pass';
        check.details.push('✅ Comprehensive state rules application system');
      } else if (statesCoverage >= 50) {
        check.status = 'warning';
  ...
  async check5_AuditLoggingFunction() {
    console.log(colors.yellow('📊 Check 5: Audit Logging Function'));
    ...
    try {
      ...
      if (auditCoverage >= 80 && auditInterface.length > 0) {
        check.status = 'pass';
  ...
  async check6_UserAcknowledgmentTracking() {
    console.log(colors.yellow('✅ Check 6: User Acknowledgment Tracking'));
    ...
    try {
      ...
      if (acknowledgmentComponents.length >= 10 && userAckComponent.length > 0) {
        check.status = 'pass';
        check.details.push('✅ Comprehensive acknowledgment tracking system');
      } else if (acknowledgmentComponents.length >= 5) {
        check.status = 'warning';
  ...
  async check7_EducationalContentAccess() {
    console.log(colors.yellow('📚 Check 7: Educational Content Accessibility'));
    ...
    try {
      ...
      if (totalEducationalContent >= 10 && accessibilityCoverage >= 60) {
        check.status = 'pass';
        check.details.push('✅ Educational content is well-organized and accessible');
      } else if (totalEducationalContent >= 5) {
        check.status = 'warning';
  ...
  async check8_ReferralSystemDisclaimers() {
    console.log(colors.yellow('🔄 Check 8: Referral System Disclaimers'));
  ...
  async searchInFiles(dir, pattern) {
    const matches = [];
  ...
  printCheckResult(check) {
    ...
    if (check.recommendations && check.recommendations.length > 0) {
      ...
      check.recommendations.forEach(rec => console.log(colors.cyan(`   • ${rec}`)));
    }
...
`;
    ...
    console.log(colors.blue(`📄 Detailed report saved to: ${reportPath}`));
  }
}
...
async function main() {
  ...
  try {
    ...
    const statusColor = results.overallStatus === 'pass' ? colors.green : 
                       results.overallStatus === 'warning' ? colors.yellow : colors.red;
    ...
    console.log(`Passed: ${colors.green(results.passedChecks)}`);
    console.log(`Warnings: ${colors.yellow(results.warningChecks)}`);
    console.log(`Failed: ${colors.red(results.failedChecks)}`);
    ...
    console.error(colors.red('❌ Validation failed with error:'), error);
    process.exit(1);
  }
}
