...
export default function EditTransactionModal({ isOpen, onClose, onSuccess, caseId, transaction }: EditTransactionModalProps) {
  ...
  useEffect(() => {
    if (transaction) {
      ...
      setFormData({
        ...
        approval_status: transaction.approval_status || 'pending',
      });
    }
  }, [transaction]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${API_CONFIG.BASE_URL}/api/v1/cases/${caseId}/transactions/${transaction.id}`, {
        method: 'PATCH',
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <h2 className="text-xl font-bold text-gray-900">Edit Financial Transaction</h2>
          <button
            onClick={onClose}
