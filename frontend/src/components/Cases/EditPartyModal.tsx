...
export default function EditPartyModal({ isOpen, onClose, onSuccess, caseId, party }: EditPartyModalProps) {
  ...
  // Pre-populate form when party changes
  useEffect(() => {
    if (party) {
      setFormData({
        ...
        attorney_firm: party.attorney_firm || '',
        notes: party.notes || '',
      });
    }
  }, [party]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${API_CONFIG.BASE_URL}/api/v1/cases/${caseId}/parties/${party.id}`, {
        method: 'PATCH',
